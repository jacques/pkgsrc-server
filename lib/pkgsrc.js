...
function parsePkgInfo(lines, filename, done) {
  var info = {
    'filename': null,
    'filesize': null,
    'raw': lines.join("\n")
    ...
    switch (parts[0]) {
      ...
      case 'FILE_SIZE':
        if (info.filesize === null) {
          info.filesize = parseInt(parts[1]);
        }
      break;
      case 'PKGNAME':
  ...
  log.debug('parsed pkg_info stream for %s (%d)', info.filename, info.filesize);
  ...
  pkg_info.on('close', function(code, signal) {
    ...
    stdout.split(/\r?\n/).forEach(function(line) {
      if (line === '' && buffer.length > 0) {
        parsePkgInfo(buffer, filenames[filenames_idx++], function(err, info) {
          callback(err, info);
        });
...
Repository.prototype.mergePkgSummary = function(from_url, done) {
  var me = this;

  var http = require('http');
  var zlib = require('zlib');

  http.get(from_url, function (res) {
    if (res.statusCode === 200) {
      ...
      res.on('end', function () {
        ...
        zlib.gunzip(buf, function(err, decoded) {
          ...
          decoded.toString().split(/\r?\n/).forEach(function(line) {
            if (line === '' && buffer.length > 0) {
              parsePkgInfo(buffer, function(err, info) {
